<?php
/**
 * Email enhancements for planner metadata.
 *
 * @package SBDP
 */
...
class SBDP_Emails {
    ...
    public static function init() {
        add_action( 'woocommerce_checkout_create_order_line_item', [ __CLASS__, 'carry_meta' ], 10, 4 );
        add_action( 'woocommerce_email_after_order_table', [ __CLASS__, 'append_program' ], 20, 4 );
    }
    ...
    private static function build_summary( array $rows, WC_Order $order ) {
        ...
        $total_participants = array_sum( array_map( static function( $row ) {
            return (int) $row['participants'];
        }, $rows ) );
        $grand_total = array_sum( array_map( static function( $row ) {
