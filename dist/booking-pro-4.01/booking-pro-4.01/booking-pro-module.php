...
if ( defined( 'SBDP_FILE' ) && realpath( (string) SBDP_FILE ) !== __FILE__ ) {
	...
	$sbdp_conflict_message = sprintf(
		...
		__( 'Booking Pro 4.01 cannot run because another Booking Pro variant (%s) is already active. Deactivate the existing module before activating this build.', 'sbdp' ),
...
if ( array() !== $sbdp_missing_files ) {
	$sbdp_missing_message = sprintf(
		...
		__( 'Booking Pro 4.01 is missing required bootstrap files: %s. Reinstall the plugin package to continue.', 'sbdp' ),
