...
final class PricingRestController {
	...
	public static function registerRoutes(): void {
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => array( __CLASS__, 'canManagePricing' ),
					'args'                => array(
						'product_id' => array(
							'required'          => true,
							'type'              => 'integer',
		...
		register_rest_route(
			...
			array(
				array(
					...
					'callback'            => array( __CLASS__, 'listRules' ),
					'permission_callback' => array( __CLASS__, 'canManagePricing' ),
				),
				array(
					'methods'             => WP_REST_Server::CREATABLE,
					'callback'            => array( __CLASS__, 'createRule' ),
					'permission_callback' => array( __CLASS__, 'canManagePricing' ),
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( __CLASS__, 'listPresets' ),
					'permission_callback' => array( __CLASS__, 'canManagePricing' ),
				),
				array(
					'methods'             => WP_REST_Server::CREATABLE,
					'callback'            => array( __CLASS__, 'applyPreset' ),
					'permission_callback' => array( __CLASS__, 'canManagePricing' ),
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( __CLASS__, 'getLog' ),
					'permission_callback' => array( __CLASS__, 'canManagePricing' ),
					'args'                => array(
						'product_id' => array(
							'type'              => 'integer',
	...
	public static function listPresets(): WP_REST_Response {
		return rest_ensure_response( array( 'presets' => PricingPresetRegistry::all() ) );
	}

	public static function applyPreset( WP_REST_Request $request ) {
		...
		if ( $key === '' ) {
			return new WP_Error( 'bsp_sales_missing_key', __( 'Preset key is required.', 'sbdp' ), array( 'status' => 422 ) );
		}

		$preset = PricingPresetRegistry::get( $key );
		if ( ! $preset ) {
			return new WP_Error( 'bsp_sales_unknown_preset', __( 'Preset niet gevonden.', 'sbdp' ), array( 'status' => 404 ) );
		}
		...
		$name = isset( $body['name'] ) && $body['name'] !== '' ? sanitize_text_field( (string) $body['name'] ) : $preset['name'];
		$priority = isset( $body['priority'] ) ? absint( (int) $body['priority'] ) : (int) ( $preset['priority'] ?? 0 );
		$active   = isset( $body['active'] ) ? (int) (bool) $body['active'] : (int) ( $preset['active'] ?? 1 );
		...
		return rest_ensure_response( array( 'id' => (int) $wpdb->insert_id, 'name' => $name ) );
	}
