...
final class Pages
{
    ...
    public static function init(): void
    {
        ...
        add_action('admin_init', [__CLASS__, 'handlePrettyAdminRoutes'], 0);
    }

    public static function registerMenu(): void
    {
        $capability = self::resolveCapability();
        $unified    = defined('SBDP_SALES_UNIFIED_MENU') && SBDP_SALES_UNIFIED_MENU;
        $parentSlug = $unified ? 'sbdp-sales-hub' : 'bsp-sales';

        if (! $unified) {
            add_menu_page(
                esc_html__('Sales Hub', 'sbdp'),
                esc_html__('Sales Hub', 'sbdp'),
                $capability,
                'bsp-sales',
            ...
            add_submenu_page($parentSlug, esc_html__('Pricing & Yield', 'sbdp'), esc_html__('Pricing & Yield', 'sbdp'), $capability, 'bsp-sales', [__CLASS__, 'renderPricing']);
        }

        add_submenu_page($parentSlug, esc_html__('Channels', 'sbdp'), esc_html__('Channels', 'sbdp'), $capability, 'bsp-sales-channels', [__CLASS__, 'renderChannels']);
        add_submenu_page($parentSlug, esc_html__('Vendors & Partners', 'sbdp'), esc_html__('Vendors', 'sbdp'), $capability, 'bsp-sales-vendors', [__CLASS__, 'renderVendors']);
        add_submenu_page($parentSlug, esc_html__('Promotions & Offers', 'sbdp'), esc_html__('Promotions & Offers', 'sbdp'), $capability, 'bsp-sales-promotions', [__CLASS__, 'renderPromotions']);
    }
    ...
    public static function enqueueAssets(string $hook): void
    {
        ...
        $config = [
            ...
            'capabilities' => [
                ...
                'managePricing'    => self::userCanManageSales(),
                'manageChannels'   => self::userCanManageSales(),
            ],
        ];
    ...
    public static function handlePrettyAdminRoutes(): void
    {
        ...
        $requestUri = $_SERVER['REQUEST_URI'] ?? ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        ...
        if (strpos($parsed['path'], $adminPath) !== 0) {
            return;
        }

        $adminBase = rtrim($adminPath, '/');
        $relative  = trim(substr($parsed['path'], strlen($adminBase)), '/');

        if ($relative === '' || $relative === 'admin.php') {
            return;
        }

        $routes = array(
            'bsp-sales'           => 'bsp-sales',
            'bsp-sales-vendors'   => 'bsp-sales-vendors',
            'bsp-sales-channels'  => 'bsp-sales-channels',
            'bsp-sales-promotions'=> 'bsp-sales-promotions',
        );
        ...
        $target = add_query_arg('page', $routes[$relative], admin_url('admin.php'));
    ...
    private static function renderShell(string $screen): void
    {
        ...
        echo '<h1>' . esc_html__('Sales Hub', 'sbdp') . '</h1>';
