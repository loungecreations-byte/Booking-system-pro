...
final class YieldEngine {
	...
	public static function init(): void {
		...
		add_filter( 'woocommerce_product_get_price', array( __CLASS__, 'filterPrice' ), 20, 2 );
		add_filter( 'woocommerce_product_get_regular_price', array( __CLASS__, 'filterPrice' ), 20, 2 );
	}
	...
	public static function filterPrice( $price, WC_Product $product ) {
		...
		$csotBasePrice = self::resolveCsotBasePrice( $product );
		if ( $csotBasePrice !== null ) {
			return $csotBasePrice;
		}

		if ( method_exists( $product, 'get_meta' ) ) {
			$runtime_lock = $product->get_meta( '_sbdp_runtime_price_locked', true );
			$runtime_override = $product->get_meta( '_sbdp_runtime_price_override', true );
			if ( $runtime_lock === 'yes' && $runtime_override !== '' && is_numeric( $runtime_override ) ) {
				return (float) $runtime_override;
			}
		}
	...
	private static function resolveCsotBasePrice( WC_Product $product ): ?float {
		$type = method_exists( $product, 'get_type' ) ? (string) $product->get_type() : '';
		if ( ! in_array( $type, array( 'bookable_service', 'arrangement', 'resource' ), true ) ) {
			return null;
		}
		...
		$baseMeta = (float) get_post_meta( $productId, '_sbdp_base_price', true );
		if ( $baseMeta > 0.0 ) {
			return round( $baseMeta, 2 );
		}
	...
	private static function ensureRuleTable( string $table ): void {
