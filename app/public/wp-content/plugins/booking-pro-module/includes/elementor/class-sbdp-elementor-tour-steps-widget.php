...
/**
 * Elementor widget to render prive tour steps.
 *
...
/**
 * Outputs the linked steps for the current prive tour.
 */
class SBDP_Elementor_Tour_Steps_Widget extends Widget_Base
{
    ...
    {
        return __('Prive tour stappen', 'sbdp');
    }
    /**
     * Widget icon.
     *
    ...
    protected function render()
    {
        ...
        foreach ($steps as $index => $step) {
            ...
            printf('<article class="sbdp-tour-step-card" data-step-id="%d">', (int) $step['id']);
            printf('<header class="sbdp-tour-step-card__header"><span class="sbdp-tour-step-card__index">%s</span><h3 class="sbdp-tour-step-card__title">%s</h3></header>', esc_html((string) ( $index + 1 )), esc_html($step['title'] ?? __('Naamloze stap', 'sbdp')));
            $meta_chips = array();
            if ('yes' === $settings['show_type_badge'] && '' !== $type_label) {
                $meta_chips[] = sprintf('<span class="sbdp-tour-step-card__chip">%s</span>', esc_html($type_label));
            }

            if ('yes' === $settings['show_points'] && $points > 0) {
                $meta_chips[] = sprintf('<span class="sbdp-tour-step-card__chip">%s</span>', esc_html(sprintf(/* translators: %d: number of points. */
                    __('%d punten', 'sbdp'),
                    $points
                )));
            }
            ...
            if (! empty($meta_chips)) {
                printf('<div class="sbdp-tour-step-card__meta">%s</div>', implode('', $meta_chips));
            }

            if ('yes' === $settings['show_excerpt'] && '' !== trim($content)) {
                printf('<div class="sbdp-tour-step-card__content">%s</div>', wp_kses_post($content));
            }
