...
$combi_deals = isset($meta['combi_deals']) && is_array($meta['combi_deals']) ? array_map('intval', $meta['combi_deals']) : array();
$combi_options = isset($combi_options) && is_array($combi_options) ? $combi_options : array();
?>
<div class="sbdp-bookable-meta" data-product-id="<?php echo esc_attr($product_id); ?>">
 <p class="description">
        <?php esc_html_e('Use this screen for booking configuration. Resources, duration, people, pricing, allowed days, and weekly availability feed the booking flow directly.', 'sbdp'); ?>
   </p>
  ...
  <nav class="nav-tab-wrapper sbdp-bookable-tabs" role="tablist">
     ...
     <button type="button" class="nav-tab" data-panel="sbdp-combi-tab" aria-selected="false"><?php esc_html_e('Combi deals', 'sbdp'); ?></button>
    </nav>
    <section id="sbdp-booking-tab" class="sbdp-tab-panel is-active" role="tabpanel">
        <div class="notice notice-info inline">
            <p><?php esc_html_e('Runtime truth here: linked resources, duration, participant limits, pricing, allowed start days, and availability windows. Confirmation and cancellation toggles are operational settings and do not override WooCommerce payment or order state on their own.', 'sbdp'); ?></p>
        </div>
        <div class="sbdp-field-grid">
            ...
            <div class="sbdp-field">
                ...
                <p class="description"><?php esc_html_e('This is the main duration value that is mirrored into the live booking duration meta.', 'sbdp'); ?></p>
            </div>
            <div class="sbdp-field">
                ...
                <p class="description"><?php esc_html_e('Planner constraint only for now. Do not treat this as hard booking truth unless the runtime has been extended for it.', 'sbdp'); ?></p>
            </div>
            <div class="sbdp-field">
    ...
    <section id="sbdp-pricing-tab" class="sbdp-tab-panel" role="tabpanel" hidden>
        <?php
        $tax_class    = isset($meta['tax_class']) ? (string) $meta['tax_class'] : (string) get_post_meta($post->ID ?? 0, '_tax_class', true);
        ...
        <div class="sbdp-field-grid">
            ...
            <div class="sbdp-field">
                <label for="sbdp-tax-class"><?php esc_html_e('Tax class', 'sbdp'); ?></label>
                <select id="sbdp-tax-class" name="sbdp_bookable[tax_class]" class="select short">
                    <?php foreach ($tax_options as $key => $label) : ?>
                        <option value="<?php echo esc_attr($key); ?>" <?php selected($tax_class, $key); ?>><?php echo esc_html($label); ?></option>
                    <?php endforeach; ?>
                </select>
                <p class="description"><?php esc_html_e('Kies 21% of 9% (of een aangepaste class) voor dit bookable product.', 'sbdp'); ?></p>
            </div>
        </div>
    ...
    <section id="sbdp-availability-tab" class="sbdp-tab-panel" role="tabpanel" hidden>
     <p class="description"><?php esc_html_e('Availability defines the weekly schedule and date overrides that feed slot projection and booking rule checks. Use this for opening hours, closures, holidays, and temporary exceptions.', 'sbdp'); ?></p>
     <div class="notice notice-info inline">
        <p><?php esc_html_e('This tab belongs here because it controls when the product can be shown as available. It is schedule input, not a separate booking engine.', 'sbdp'); ?></p>
     </div>
 ...
 <section id="sbdp-combi-tab" class="sbdp-tab-panel" role="tabpanel" hidden>
        <div class="sbdp-field">
           <label for="sbdp-combi-deals"><?php esc_html_e('Combi deals', 'sbdp'); ?></label>
            <select name="sbdp_bookable[combi_deals][]" id="sbdp-combi-deals" class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e('Select bookable products', 'sbdp'); ?>">
                <?php foreach ($combi_options as $combi_option) :
                    ?>
                    <?php
                    $combi_id    = isset($combi_option['id']) ? (int) $combi_option['id'] : 0;
                    $combi_title = isset($combi_option['title']) ? $combi_option['title'] : '';
                    if ($combi_id <= 0 || '' === trim($combi_title)) {
                        continue;
                    }
                    ?>
                 <option value="<?php echo esc_attr((string) $combi_id); ?>" <?php selected(in_array($combi_id, $combi_deals, true), true); ?>>
                       <?php echo esc_html($combi_title); ?>
                    </option>
                    <?php
                endforeach; ?>
           </select>
          <p class="description"><?php esc_html_e('Select bookable products to offer as optional combi deals on the product page.', 'sbdp'); ?></p>
        </div>
    </section>
