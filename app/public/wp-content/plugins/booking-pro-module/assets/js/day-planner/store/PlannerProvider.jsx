...
export function PlannerProvider({ bootConfig, children }) {
  ...
  const aiSuggestionsAppliedRef = React.useRef(false);
  ...
  useEffect(() => {
    ...
    // 🤖 Check for pre-fetched AI suggestions from Home Widget
    const aiSuggestions = window.SBDP_AI_SUGGESTIONS || (() => {
      try {
        const stored = sessionStorage.getItem('sbdp_ai_suggestions');
    ...
    if (homeWidgetAppliedRef.current) {
      console.log('✅ Auto-fill already applied, skipping');
      return;
    }
    
    // CRITICAL: Clear any existing draft plan from localStorage to start fresh
    if (typeof window !== 'undefined' && window.localStorage) {
      const hadDraft = window.localStorage.getItem('sbdpPlannerDraftV1');
      if (hadDraft) {
        console.log('🗑️ Clearing existing draft plan from localStorage');
        window.localStorage.removeItem('sbdpPlannerDraftV1');
      }
    }
    
    // Wait for config and products
    if (!state.config || state.loading.products) {
      console.log('⏳ Waiting for config or products...', { 
        hasConfig: !!state.config, 
    ...
    // CRITICAL: Wait for products to actually load (not just loading=false)
    if (state.products.length === 0) {
      if (!homeWidgetNoProductsRef.current) {
        homeWidgetNoProductsRef.current = true;
        showToast("Nog geen activiteiten beschikbaar om je dag automatisch te vullen.");
      }
      console.log('⏳ Products not loaded yet, waiting...');
      return;
    }
    
    console.log(`✅ Ready! ${state.products.length} products available`);
    
    // Start planning if not started
    if (!state.plan.days.length) {
      const participants = parseInt(widgetData.count, 10) || 2;
      console.log(`?? Starting plan: ${widgetData.visitDate} with ${participants} people`);
      if (!homeWidgetStartRef.current) {
        homeWidgetStartRef.current = true;
        dispatch({
          type: ACTIONS.START_PLANNING,
          payload: { 
            date: widgetData.visitDate, 
            participants, 
            config: state.config 
          },
        });
    ...
    // Wait for addActivity to be ready
    const addActivityFn = addActivityRef.current;
    if (typeof addActivityFn !== "function") {
      console.log('⏳ Waiting for addActivity function...');
      return;
    }
    
    // Clear existing plan items if we have home widget data
    if (state.plan.items && state.plan.items.length > 0) {
      console.log('🗑️ Clearing existing plan items before auto-fill');
      dispatch({ type: ACTIONS.CLEAR_PLAN });
    }
    
    // Mark as applied BEFORE generating to prevent loops
    homeWidgetAppliedRef.current = true;
    console.log('🚀 Starting auto-fill...');
    
    // Get current products from state (avoid stale closure)
    const currentProducts = state.products;
    
    // DEBUG: Log first 3 products to see their structure
    console.log('🔍 Sample products:', currentProducts.slice(0, 3).map(p => ({
      id: p.id,
      name: p.name || p.title,
    ...
    // Determine theme from audience + vibe
    const theme = resolveThemeFromHints({
      vibe: widgetData.vibe,
      audience: widgetData.audience,
      duration: widgetData.duration
    });
    console.log(`🎨 Resolved theme: ${theme}`);
    
    // AUDIENCE-BASED SCORING PREFERENCES
    const audiencePreferences = {
      'collegas': {
        boost: {
          'teambuilding': 50, 'workshop': 40, 'vergaderen': 30,
          'vergaderboot': 40, 'zakelijk': 30, 'escape': 45
        },
        penalty: { 'romantisch': -50, 'kinderen': -30 }
      },
      'vrienden': {
        boost: {
          'tour': 40, 'actief': 50, 'e-chopper': 60, 'e-step': 50,
          'escape': 45, 'pubquiz': 40, 'quiz': 35, 'fietstocht': 45
        },
        penalty: { 'zakelijk': -30, 'kinderen': -20 }
      },
      'gezin': {
        boost: {
          'kinderen': 60, 'kidsproof': 50, 'familie': 40, 'speeltuin': 45
        },
        penalty: { 'alcohol': -40, 'zakelijk': -50 }
      },
      'partner': {
        boost: {
          'romantisch': 60, 'diner': 50, 'wandeling': 40, 'museum': 35
        },
        penalty: { 'teambuilding': -40, 'groep': -30 }
      },
      'solo': {
        boost: {
          'museum': 50, 'wandeling': 45, 'cultuur': 40, 'kunst': 40
        },
        penalty: { 'teambuilding': -60, 'groep': -40 }
      },
      'gemengd': {
        boost: {
          'lunch': 30, 'borrel': 30, 'wandeling': 25, 'tour': 25
        },
        penalty: { 'teambuilding': -10 }
      }
    };
    
    // VIBE-BASED SCORING PREFERENCES
    const vibePreferences = {
      'bourgondisch': {
        boost: {
          'bossche bol': 70, 'lunch': 60, 'proeverij': 80, 'bier': 60,
          'wijn': 60, 'diner': 70, 'restaurant': 60, 'high tea': 55, 'borrel': 50
        },
        penalty: { 'sportief': -30, 'actief': -20 }
      },
      'cultuur': {
        boost: {
          'museum': 80, 'jeroen bosch': 90, 'kunst': 70, 'geschiedenis': 60,
          'rondleiding': 60, 'gids': 50
        },
        penalty: { 'sport': -40, 'feest': -30 }
      },
      'shoppen': {
        boost: {
          'winkel': 80, 'shopping': 80, 'mode': 60, 'markt': 50, 'outlet': 60
        },
        penalty: { 'museum': -20, 'natuur': -30 }
      },
      'kidsproof': {
        boost: {
          'kinderen': 90, 'speeltuin': 80, 'familie': 60, 'pretpark': 70, 'dierentuin': 70
        },
        penalty: { 'alcohol': -60, 'zakelijk': -50 }
      },
      'actief': {
        boost: {
          'actief': 60, 'sport': 50, 'tour': 35, 'wandeling': 35, 'boot': 30, 'escape': 40
        },
        penalty: { 'diner': -25, 'restaurant': -20 }
      },
      'klassiek': {
        boost: {
          'klassiek': 40, 'museum': 60, 'cultuur': 50, 'histor': 50, 'wandeling': 25
        },
        penalty: { 'feest': -20 }
      },
      'relaxed': {
        boost: {
          'borrel': 45, 'lunch': 35, 'proeverij': 35, 'wandeling': 30, 'boot': 25
        },
        penalty: { 'extreem': -40, 'sportief': -30 }
      },
      'verrassend': {
        boost: {
          'escape': 70, 'quiz': 60, 'workshop': 60, 'uniek': 80,
          'bijzonder': 70, 'origineel': 60
        },
        penalty: { 'standaard': -40, 'basic': -50 }
      }
    };
    
    // FUNCTION: Build alternative pool for a time slot
    const buildAlternativePool = (timeSlot, duration, audience, vibe, allProducts) => {
      try {
        console.log('[buildAlternativePool] START - Building for:', { timeSlot, duration, audience, vibe, productCount: allProducts?.length });
        
        if (!allProducts || !Array.isArray(allProducts)) {
          console.warn('[buildAlternativePool] No products array provided');
          return [];
        }
        
        if (allProducts.length === 0) {
          console.log('[buildAlternativePool] Empty products array');
          return [];
        }
        ...
        allProducts.forEach((product, idx) => {
          try {
            if (!product) {
              console.warn(`[buildAlternativePool] Null product at index ${idx}`);
              return;
            }
            
            const profile = getProductTimeProfile(product);
            
            if (!profile || !profile.preferredTimes) {
              console.warn(`[buildAlternativePool] No profile for product ${product.id}`);
              return;
            }
            
            // Check if product can fit in this time slot
            if (!profile.preferredTimes.includes(timeSlot)) return;
            ...
            // AUDIENCE SCORING
            if (audience && audiencePreferences[audience]) {
              const audiencePref = audiencePreferences[audience];
              if (audiencePref.boost) {
                Object.entries(audiencePref.boost).forEach(([keyword, points]) => {
                  if (name.includes(keyword)) score += points;
                });
              }
              if (audiencePref.penalty) {
                Object.entries(audiencePref.penalty).forEach(([keyword, points]) => {
                  if (name.includes(keyword)) score += points;
                });
              }
            }
            ...
            // VIBE SCORING
            if (vibe && vibePreferences[vibe]) {
              const vibePref = vibePreferences[vibe];
              if (vibePref.boost) {
                Object.entries(vibePref.boost).forEach(([keyword, points]) => {
                  if (name.includes(keyword)) score += points;
                });
              }
              if (vibePref.penalty) {
                Object.entries(vibePref.penalty).forEach(([keyword, points]) => {
                  if (name.includes(keyword)) score += points;
                });
              }
            }
            
            // TIME PROFILE SCORING (prefer exact match)
            if (profile.preferredTimes[0] === timeSlot) score += 20;
            ...
            console.error(`[buildAlternativePool] Error processing product ${idx}:`, productErr);
          }
        });
        
        console.log(`[buildAlternativePool] Collected ${alternatives.length} alternatives before sorting`);
        ...
        const result = alternatives
          ...
          .map(alt => ({
            id: alt.product.id,
            name: alt.product.name || alt.product.title,
            score: alt.score,
            duration: alt.profile.duration,
            category: alt.profile.slot,
          resource_id: alt.product.resource_id
        }));
        
        console.log(`[buildAlternativePool] Built ${result.length} alternatives for slot ${timeSlot}`);
        return result;
        
      } catch (error) {
        console.error('[buildAlternativePool] FATAL Error:', error);
        return [];
      }
    };
    
    // DEBUG: Check what tokens products actually have
    console.log('🔍 Product tokens sample:', currentProducts.slice(0, 5).map(p => ({
      name: p.name || p.title,
      tokens: getProductCategoryTokens(p),
      categories: p.categories,
      category_slugs: p.category_slugs
    })));
    
    // Get requested participants for filtering
    const requestedParticipants = parseInt(widgetData.count, 10) || 2;
    const audience = widgetData.audience || 'vrienden';
    console.log(`👥 Filtering products for ${requestedParticipants} participants, audience: ${audience}`);
    
    // 👥 AUDIENCE-BASED FILTERING RULES
    // Each audience has preferred and avoided product keywords
    const audienceRules = {
      partner: {
        boost: ['romantisch', 'wijn', 'diner', 'boot', 'proeverij', 'walking dinner', 'high tea'],
        avoid: ['pubquiz', 'escape', 'workshop', 'carnaval', 'quiz', 'team'],
        minScore: 0 // Lower threshold for filtering
      },
      gezin: {
        boost: ['workshop', 'museum', 'rondleiding', 'stadswandeling', 'boot', 'fietsen'],
        avoid: ['wijnproeverij', 'bierproeverij', 'pubquiz', 'borrel', 'quiz'],
        minScore: 0
      },
      vrienden: {
        boost: ['bierproeverij', 'pubquiz', 'escape', 'e-chopper', 'boot', 'borrel', 'quiz'],
        avoid: ['romantisch'],
        minScore: 0
      },
      collegas: {
        boost: ['workshop', 'escape', 'walking dinner', 'rondleiding', 'team', 'proeverij'],
        avoid: ['romantisch', 'kids', 'kinderen'],
        minScore: 0
      },
      solo: {
        boost: ['stadswandeling', 'museum', 'rondleiding', 'cultuur', 'kunst'],
        avoid: ['team', 'groep', 'escape'],
        minScore: 0
      }
    };
    
    const rules = audienceRules[audience] || audienceRules.vrienden;
    
    // 🌦️ SEIZOENSLOGICA: Prefer indoor/outdoor based on month
    // Winter months (Nov-Feb): prefer indoor activities
    // Summer months (May-Aug): prefer outdoor activities
    // Shoulder months: neutral
    const getSeasonPreferences = () => {
      const visitDateStr = widgetData.visitDate || widgetData.date;
      const date = visitDateStr ? new Date(visitDateStr) : new Date();
      const month = date.getMonth(); // 0-11
      
      // Indoor keywords
      const indoorKeywords = ['museum', 'workshop', 'proeverij', 'diner', 'restaurant', 'escape', 'high tea', 'lunch'];
      // Outdoor keywords  
      const outdoorKeywords = ['boot', 'boottocht', 'fietsen', 'e-chopper', 'wandeling', 'stadswandeling', 'fietstocht'];
      
      // Winter: Nov (10), Dec (11), Jan (0), Feb (1)
      if (month >= 10 || month <= 1) {
        return { boost: indoorKeywords, avoid: outdoorKeywords, label: 'winter' };
      }
      // Summer: May (4), Jun (5), Jul (6), Aug (7)
      if (month >= 4 && month <= 7) {
        return { boost: outdoorKeywords, avoid: [], label: 'zomer' }; // Don't avoid indoor in summer
      }
      // Shoulder months: Mar, Apr, Sep, Oct - neutral
      return { boost: [], avoid: [], label: 'tussenseizoen' };
    };
    
    const seasonPrefs = getSeasonPreferences();
    console.log(`🌦️ Season preferences (${seasonPrefs.label}):`, seasonPrefs);
    
    // SMART FILTERING: Remove unsuitable products
    const excludeKeywords = ['dagje den bosch', 'arrangement', 'pakket', 'compleet'];
    const validProducts = currentProducts.filter(product => {
      const name = (product.name || product.title || '').toLowerCase();
      // Exclude generic/package products
      if (excludeKeywords.some(keyword => name.includes(keyword))) {
        return false;
      }
      
      // CRITICAL: Check if product supports the requested number of participants.
      // Only applies to products with a real group capacity (max > 1).
      // Per-person items (max=1, e.g. food/drink) are always includable.
      if (product.people?.enabled && (product.people?.max ?? 1) > 1) {
        const min = Math.max(1, product.people.min || 1);
        const max = Math.max(min, product.people.max || min);
        if (requestedParticipants < min || requestedParticipants > max) {
          return false;
        }
      }
      
      // 👥 AUDIENCE FILTER: Exclude products that don't fit the audience
      if (rules.avoid.some(keyword => name.includes(keyword))) {
        return false;
      }
    ...
    // 👥 AUDIENCE SCORING: Boost products that match audience preferences
    validProducts.forEach(product => {
      const name = (product.name || product.title || '').toLowerCase();
      product._audienceScore = 0;
      product._seasonScore = 0;
      
      // Add score for each matching boost keyword
      rules.boost.forEach(keyword => {
        if (name.includes(keyword)) {
          product._audienceScore += 10;
        }
      });
      
      // 🌦️ SEASON SCORING: Boost products matching season preference
      seasonPrefs.boost.forEach(keyword => {
        if (name.includes(keyword)) {
          product._seasonScore += 8; // Slightly less than audience
        }
      });
      // Penalty for season-inappropriate activities
      seasonPrefs.avoid.forEach(keyword => {
        if (name.includes(keyword)) {
          product._seasonScore -= 5; // Small penalty, don't exclude
        }
      });
      
      // Combined score
      product._totalScore = (product._audienceScore || 0) + (product._seasonScore || 0);
    });
    
    // Sort by combined score (higher = better match)
    validProducts.sort((a, b) => (b._totalScore || 0) - (a._totalScore || 0));
    
    console.log(`✅ ${validProducts.length}/${currentProducts.length} products for ${audience} (${seasonPrefs.label})`);
    console.log(`🔝 Top 5 scored:`, validProducts.slice(0, 5).map(p => `${p.name || p.title} (${p._totalScore})`));
    
    // TIME-AWARE PRODUCT PROFILES
    // Each product type has preferred time slots (in minutes from midnight)
    // Slots: 570 (09:30), 600 (10:00), 630 (10:30), 720 (12:00), 810 (13:30), 960 (16:00), 1050 (17:30), 1080 (18:00), 1110 (18:30), 1230 (20:30), 1260 (21:00)
    const productTimeProfiles = {
      // Morning treats & coffee - 09:30 slot
      'bossche bol': { preferredTimes: [570], duration: 30, slot: 'breakfast' },
      'koffie': { preferredTimes: [570, 600], duration: 30, slot: 'breakfast' },
      'ontbijt': { preferredTimes: [570, 600], duration: 30, slot: 'breakfast' },
      
      // Tours & walking - 10:00/10:30 or 13:30 slots
      'stadswandeling': { preferredTimes: [600, 630, 810], duration: 90, slot: 'tour' },
      'rondleiding': { preferredTimes: [600, 630, 810], duration: 90, slot: 'tour' },
      'wandeling': { preferredTimes: [600, 630, 810], duration: 90, slot: 'tour' },
      'fietstocht': { preferredTimes: [630, 810], duration: 120, slot: 'tour' },
      'jeroen bosch': { preferredTimes: [600, 630, 810], duration: 60, slot: 'culture' },
      'gids': { preferredTimes: [600, 630, 810], duration: 90, slot: 'tour' },
      
      // Lunch window - 12:00 slot
      'lunch': { preferredTimes: [720], duration: 60, slot: 'lunch' },
      'lunchproeverij': { preferredTimes: [720], duration: 60, slot: 'lunch' },
      'proeverij': { preferredTimes: [720, 1050], duration: 60, slot: 'lunch' }, // Lunch OR early-evening borrel
      
      // Bier/wijn proeverij - EVENING ONLY (borrel 17:30 or late 21:00)
      'bierproeverij': { preferredTimes: [1260, 1050], duration: 60, slot: 'activity' },
      'wijnproeverij': { preferredTimes: [1260, 1050], duration: 60, slot: 'activity' },
      
      // High Tea - 16:00 slot
      'high tea': { preferredTimes: [960], duration: 60, slot: 'activity' },
      'hightea': { preferredTimes: [960], duration: 60, slot: 'activity' },
      
      // Main activities - 13:30 slot (after lunch) or 10:00/10:30
      'boot': { preferredTimes: [810, 630, 900], duration: 120, slot: 'tour' },
      'boottocht': { preferredTimes: [810, 630, 900], duration: 120, slot: 'tour' },
      'e-chopper': { preferredTimes: [810, 630], duration: 120, slot: 'tour' },
      'workshop': { preferredTimes: [810], duration: 90, slot: 'activity' },
      'bossche bollen workshop': { preferredTimes: [810], duration: 90, slot: 'activity' },
      'carnaval': { preferredTimes: [810, 630], duration: 60, slot: 'activity' },
      'oeteldonk': { preferredTimes: [810, 630], duration: 60, slot: 'activity' },
      
      // Museums & culture - morning or afternoon
      'museum': { preferredTimes: [630, 690, 810], duration: 90, slot: 'culture' },
      'kunst': { preferredTimes: [630, 690, 810], duration: 90, slot: 'culture' },
      
      // Dinner - 18:00 or 18:30 slot
      'diner': { preferredTimes: [1080, 1110], duration: 120, slot: 'dinner' },
      '3 gangen': { preferredTimes: [1080, 1110], duration: 120, slot: 'dinner' },
      'avondeten': { preferredTimes: [1080, 1110], duration: 120, slot: 'dinner' },
      'restaurant': { preferredTimes: [1080, 1110], duration: 120, slot: 'dinner' },
      'walking dinner': { preferredTimes: [1080, 1110], duration: 120, slot: 'dinner' },
      'wandeldiner': { preferredTimes: [1080, 1110], duration: 120, slot: 'dinner' },
      
      // Borrel - 17:30 (avond-start), 16:00 (middag-eind), or 20:30 (na-diner)
      'borrel': { preferredTimes: [1050, 960, 1020, 1230], duration: 60, slot: 'activity' },
      
      // Evening activities - 21:00 slot (after dinner)
      'pubquiz': { preferredTimes: [1260], duration: 90, slot: 'activity' },
      'quiz': { preferredTimes: [1260], duration: 90, slot: 'activity' },
      
      // Active/entertainment - afternoon
      'escape': { preferredTimes: [810], duration: 90, slot: 'activity' },
    };
    
    // Helper: get time profile for product
    const getProductTimeProfile = (product) => {
      const name = (product.name || product.title || '').toLowerCase();
      
      // Find matching profile
      for (const [keyword, profile] of Object.entries(productTimeProfiles)) {
        if (name.includes(keyword)) {
          return profile;
        }
      }
      ...
      return { 
        preferredTimes: [600, 840, 1020], // 10:00, 14:00, 17:00
        duration: 90, 
        slot: 'activity' 
      };
    };
    
    // INTELLIGENT CATEGORIZATION by product name/type
    ...
    validProducts.forEach(product => {
      ...
      // Breakfast items (rare, but possible)
      if (name.includes('ontbijt') || name.includes('brunch') || name.includes('bossche bol')) {
        categorizedProducts.breakfast.push(product);
      }
      // EVENING proeverij - bier/wijn proeverij are evening activities, NOT lunch
      else if (name.includes('bierproeverij') || name.includes('wijnproeverij')) {
        categorizedProducts.activity.push(product); // Goes to evening activity slot
      }
      // Lunch items - flexible matching (but exclude bier/wijn proeverij)
      else if (name.includes('lunch') || name.includes('lunchpakket') || 
               name.includes('lunchproeverij') ||
               (name.includes('proeverij') && !name.includes('bier') && !name.includes('wijn')) ||
               name.includes('hapje') ||
               (name.includes('eten') && !name.includes('avond') && !name.includes('diner'))) {
        // Exclude high tea and dinner items
        if (!name.includes('high tea') && !name.includes('hightea') && 
            !name.includes('diner') && !name.includes('avondeten')) {
          categorizedProducts.lunch.push(product);
        }
      }
      // Dinner items - must explicitly mention dinner/avond
      else if (name.includes('diner') || name.includes('avondeten') || name.includes('restaurant') || name.includes('walking dinner')) {
        categorizedProducts.dinner.push(product);
      }
      // Tea/coffee - afternoon activity
      else if (name.includes('high tea') || name.includes('hightea') || name.includes('thee')) {
        categorizedProducts.activity.push(product); // Hightea = afternoon activity
      }
      // Cultural activities
      else if (name.includes('museum') || name.includes('kunst') || name.includes('historie') || name.includes('jeroen bosch') ||
               tokens.some(t => ['cultuur', 'museum', 'kunst'].includes(t))) {
        categorizedProducts.culture.push(product);
      }
      // Tours & active
      else if (name.includes('rondleiding') || name.includes('tour') || name.includes('wandeling') ||
               name.includes('fietstocht') || name.includes('boot')) {
        categorizedProducts.tour.push(product);
      }
      // Workshops & activities
      else if (name.includes('workshop') || name.includes('escape') || name.includes('quiz') || name.includes('carnaval')) {
        categorizedProducts.activity.push(product);
      }
      // Everything else = general activity
      else {
        categorizedProducts.activity.push(product);
      }
    });
    ...
    console.log('📦 Categorized products:', {
      breakfast: categorizedProducts.breakfast.length,
      lunch: categorizedProducts.lunch.length,
      dinner: categorizedProducts.dinner.length,
      culture: categorizedProducts.culture.length,
      tour: categorizedProducts.tour.length,
      activity: categorizedProducts.activity.length
    });
    
    // Check if we have enough products
    const totalAvailable = Object.values(categorizedProducts).reduce((sum, arr) => sum + arr.length, 0);
    if (totalAvailable < 4) {
      console.warn('⚠️ Not enough valid products for full day plan');
      return;
    }
    ...
    // Generate 4 activities with SMART TIME SCHEDULING
    const activitiesAdded = [];
    const usedProductIds = new Set();
    const occupiedTimeSlots = []; // Track what times are taken
    
    // Helper: check if time slot is available
    const isTimeSlotAvailable = (startMinutes, durationMinutes) => {
      const endMinutes = startMinutes + durationMinutes;
      return !occupiedTimeSlots.some(slot => {
        const slotEnd = slot.start + slot.duration;
        // Check for overlap
        return (startMinutes < slotEnd && endMinutes > slot.start);
      });
    };
    
    // Helper: find best available time for product
    const findBestTime = (product) => {
      const profile = getProductTimeProfile(product);
      
      // Try preferred times first
      for (const preferredTime of profile.preferredTimes) {
        if (isTimeSlotAvailable(preferredTime, profile.duration)) {
          return { time: preferredTime, duration: profile.duration };
        }
      }
      
      // No preferred time available, return null
      return null;
    };

    // Helper: find available time within a window (start/end minutes)
    const findTimeInWindow = (product, windowStart, windowEnd) => {
      const profile = getProductTimeProfile(product);
      if (!profile || !profile.preferredTimes) {
        return null;
      }
      for (const preferredTime of profile.preferredTimes) {
        const endTime = preferredTime + profile.duration;
        if (preferredTime < windowStart || endTime > windowEnd) {
          continue;
        }
        if (isTimeSlotAvailable(preferredTime, profile.duration)) {
          return { time: preferredTime, duration: profile.duration };
    ...
    // Helper: pick from category with time validation
    const pickFromCategoryWithTime = (categoryName) => {
      const pool = categorizedProducts[categoryName] || [];
      const available = pool.filter(p => !usedProductIds.has(p.id));
      
      // Try each product until we find one with available time slot
      for (const product of available) {
        const timeSlot = findBestTime(product);
        if (timeSlot) {
          return { product, timeSlot };
        }
      }
      return null;
    };
    
    // Helper: pick from multiple categories
    const pickFromCategoriesWithTime = (...categories) => {
      for (const cat of categories) {
        const result = pickFromCategoryWithTime(cat);
        if (result) return result;
      }
      return null;
    };
    
    // DECLARATIVE DURATION SCHEDULES
    // Each slot has: time (minutes from midnight), duration, categories (fallback chain), label, emoji
    const DURATION_SCHEDULES = {
      // 🌅 OCHTEND (09:00 - 13:00) - Ideaal voor vroege vogels, cultuurliefhebbers
      'ochtend': [
        { time: 570, duration: 30, categories: ['breakfast'], label: 'Bossche Bol & Koffie', emoji: '☕' },     // 09:30-10:00
        { time: 600, duration: 90, categories: ['tour', 'culture'], label: 'Stadswandeling', emoji: '🚶' },    // 10:00-11:30
        { time: 720, duration: 60, categories: ['lunch'], label: 'Lunchproeverij', emoji: '🍽️' }              // 12:00-13:00
      ],
      
      // ☀️ MIDDAG (12:00 - 18:00) - Ideaal voor families, relaxte start
      'middag': [
        { time: 720, duration: 60, categories: ['lunch'], label: 'Lunch', emoji: '🍽️' },                       // 12:00-13:00
        { time: 810, duration: 120, categories: ['activity', 'tour'], label: 'Hoofdactiviteit', emoji: '🎨' }, // 13:30-15:30
        { time: 960, duration: 60, categories: ['activity', 'lunch'], label: 'High Tea / Borrel', emoji: '🫖' } // 16:00-17:00
      ],
      
      // 🌙 AVOND (17:00 - 23:00) - Ideaal voor date night, vrijdagavond met vrienden
      'avond': [
        { time: 1050, duration: 60, categories: ['activity', 'lunch'], label: 'Borrel', emoji: '🍸' },          // 17:30-18:30
        { time: 1110, duration: 120, categories: ['dinner'], label: 'Diner', emoji: '🍷' },                     // 18:30-20:30
        { time: 1260, duration: 90, categories: ['activity'], label: 'Avondactiviteit', emoji: '🎉' }          // 21:00-22:30
      ],
      
      // 🌞 HELE DAG (09:00 - 22:00) - Complete ervaring
      'hele-dag': [
        { time: 570, duration: 30, categories: ['breakfast'], label: 'Bossche Bol & Koffie', emoji: '☕' },     // 09:30-10:00
        { time: 630, duration: 90, categories: ['tour', 'culture'], label: 'Stadswandeling', emoji: '🚶' },    // 10:30-12:00
        { time: 720, duration: 60, categories: ['lunch'], label: 'Lunch', emoji: '🍽️' },                       // 12:00-13:00
        { time: 810, duration: 120, categories: ['activity', 'tour'], label: 'Hoofdactiviteit', emoji: '🎨' }, // 13:30-15:30
        { time: 960, duration: 60, categories: ['activity', 'lunch'], label: 'High Tea', emoji: '🫖' },        // 16:00-17:00
        { time: 1080, duration: 120, categories: ['dinner'], label: 'Diner', emoji: '🍷' },                    // 18:00-20:00
        { time: 1230, duration: 60, categories: ['activity'], label: 'Avondactiviteit', emoji: '🎉' }          // 20:30-21:30
      ],
      
      // 🏨 WEEKEND (2 dagen) - Uitgebreid programma
      'weekend': {
        day1: [
          { time: 570, duration: 30, categories: ['breakfast'], label: 'Koffie & Gebak', emoji: '☕' },
          { time: 630, duration: 90, categories: ['tour'], label: 'Stadswandeling', emoji: '🚶' },
          { time: 720, duration: 60, categories: ['lunch'], label: 'Lunch', emoji: '🍽️' },
          { time: 810, duration: 120, categories: ['activity'], label: 'Hoofdactiviteit', emoji: '🎨' },
          { time: 960, duration: 60, categories: ['activity', 'lunch'], label: 'High Tea', emoji: '🫖' },
          { time: 1080, duration: 120, categories: ['dinner'], label: 'Diner', emoji: '🍷' },
          { time: 1230, duration: 90, categories: ['activity'], label: 'Avondactiviteit', emoji: '🎉' }
        ],
        day2: [
          { time: 600, duration: 60, categories: ['breakfast', 'lunch'], label: 'Brunch', emoji: '🥐' },
          { time: 690, duration: 90, categories: ['culture'], label: 'Cultuur & Museum', emoji: '🖼️' },
          { time: 810, duration: 60, categories: ['lunch'], label: 'Lichte Lunch', emoji: '🥗' },
          { time: 900, duration: 90, categories: ['tour', 'activity'], label: 'Boottocht / Tour', emoji: '🚤' },
          { time: 1020, duration: 60, categories: ['activity'], label: 'Borrel', emoji: '🍻' },
          { time: 1110, duration: 90, categories: ['dinner'], label: 'Afsluitend Diner', emoji: '🍷' }
        ]
      }
    };
    
    const duration = widgetData.duration || 'hele-dag';
    const plannedActivities = [];
    
    console.log(`📅 Building ${duration} schedule...`);
    
    // 🤖 CHECK FOR PRE-FETCHED AI SUGGESTIONS
    // If AI suggestions are available and fresh (< 5 min old), use them instead of local generation
    if (aiSuggestions && aiSuggestions.itinerary && aiSuggestions.itinerary.length > 0 && !aiSuggestionsAppliedRef.current) {
      const suggestionsAge = Date.now() - (aiSuggestions.generatedAt || 0);
      const MAX_AGE = 5 * 60 * 1000; // 5 minutes
      
      if (suggestionsAge < MAX_AGE) {
        console.log('🤖 Using pre-fetched AI suggestions!', aiSuggestions.itinerary);
        aiSuggestionsAppliedRef.current = true;
        ...
        void (async () => {
          ...
          for (const item of aiSuggestions.itinerary) {
            const productId = item.product_id || item.productId;
            const startTime = item.start_time || item.startTime;

            if (!productId || !startTime) {
              console.warn('🤖 Skipping invalid AI item:', item);
              continue;
            }

            const product = currentProducts.find(p => p.id === productId);
            if (!product) {
              console.warn(`🤖 Product ${productId} not found in loaded products`);
              continue;
            }
            ...
            if (!resolvedStartTime) {
              console.warn(`🤖 Skipping AI suggestion due to unresolved availability: ${productId}`);
              continue;
            }

            console.log(`🤖 Adding AI suggestion: ${product.name || product.title} @ ${resolvedStartTime}`);

            const added = addActivityFn(
              { productId, dayIndex: 0, startTime: resolvedStartTime },
              { resourceId: product.resource_id ?? null }
            );
          ...
          if (aiAdded > 0) {
            console.log(`🤖 Successfully added ${aiAdded} AI-suggested activities`);
            showToast(`${aiAdded} activiteiten toegevoegd op basis van je voorkeuren!`);

            try {
              sessionStorage.removeItem('sbdp_ai_suggestions');
              delete window.SBDP_AI_SUGGESTIONS;
            } catch (e) {
              // ignore
            }
          }
        ...
        return; // Skip regular generation
      } else {
        console.log('🤖 AI suggestions too old, falling back to local generation');
      }
    }
    
    // Get schedule for this duration
    console.log(`📅 Building schedule for duration: "${duration}"`);
    console.log(`📅 Available durations:`, Object.keys(DURATION_SCHEDULES));
    
    let schedule = DURATION_SCHEDULES[duration];
    
    if (!schedule) {
      console.error(`❌ No schedule found for duration "${duration}"!`);
      return;
    }
    
    console.log(`📅 Schedule has ${schedule.length || 0} slots`);
    
    // For weekend, we need to handle 2 days
    if (duration === 'weekend') {
      // TODO: Implement 2-day scheduling
      console.log('🏖️ WEEKEND: 2-day schedule not yet implemented, using day1 for now');
      schedule = schedule.day1;
    }
    
    // Build activities from schedule
    schedule.forEach((slot, index) => {
      console.log(`\n${slot.emoji} Slot ${index + 1}: ${slot.label} @ ${minutesToTime(slot.time)} (${slot.duration}min slot)`);
      console.log(`   Categories to try: ${slot.categories.join(' → ')}`);
      
      // Try each category in order (fallback chain)
      let picked = null;
      for (const category of slot.categories) {
        const pool = categorizedProducts[category] || [];
        const available = pool.filter(p => !usedProductIds.has(p.id));
        console.log(`   - ${category}: ${available.length} products available`);
        
        // PASS 1: Find product that prefers this exact time slot
        for (const product of available) {
          const profile = getProductTimeProfile(product);
          const realDuration = getDurationMinutes(product) || profile.duration || slot.duration;
          
          // Check if product's preferred times include this slot time (exact match)
          if (profile.preferredTimes.includes(slot.time)) {
            if (isTimeSlotAvailable(slot.time, realDuration)) {
              picked = { 
                product, 
                timeSlot: { time: slot.time, duration: realDuration },
                label: slot.label,
                emoji: slot.emoji
              };
              console.log(`   ✅ Picked (exact): ${product.name || product.title} (${realDuration}min)`);
              break;
            }
          }
        }
        ...
        // PASS 2: Find product with nearby preferred time (within 60 min)
        const TIME_TOLERANCE = 60; // Allow 60 minute tolerance
        for (const product of available) {
          const profile = getProductTimeProfile(product);
          const realDuration = getDurationMinutes(product) || profile.duration || slot.duration;
          
          // Check if any preferred time is within tolerance of slot time
          const nearbyMatch = profile.preferredTimes.some(pt => Math.abs(pt - slot.time) <= TIME_TOLERANCE);
          if (nearbyMatch) {
            if (isTimeSlotAvailable(slot.time, realDuration)) {
              picked = { 
                product, 
              ...
              console.log(`   ✅ Picked (nearby): ${product.name || product.title} (${realDuration}min)`);
              break;
            }
          }
        }
        ...
        // PASS 3: Fallback - just pick any available product from category
        for (const product of available) {
          const profile = getProductTimeProfile(product);
          const realDuration = getDurationMinutes(product) || profile.duration || slot.duration;
          ...
          if (isTimeSlotAvailable(slot.time, realDuration)) {
            ...
            console.log(`   ✅ Picked (fallback): ${product.name || product.title} (${realDuration}min)`);
            break;
          }
        }
        if (picked) break;
      ...
      if (!picked) {
        console.warn(`   ❌ No product found for ${slot.label}`);
      } else {
        plannedActivities.push(picked);
      }
    });
    
    // Sort by time to ensure chronological order
    plannedActivities.sort((a, b) => a.timeSlot.time - b.timeSlot.time);
    
    console.log(`\n📊 Planned ${plannedActivities.length} activities`);
    
    void (async () => {
      const plannerDate = state.plan.days[0]?.date || state.form.date;
      const plannerParticipants = canonicalParticipants;

      for (const { product, timeSlot, label, emoji } of plannedActivities) {
        const startTime = minutesToTime(timeSlot.time);
        const resolvedStartTime = await resolveStartTimeAgainstAvailability({
          product,
          desiredStartTime: startTime,
          date: plannerDate,
        ...
        if (!resolvedStartTime) {
          console.warn(`   ❌ Skipping due to unresolved availability: ${product.name || product.title}`);
          continue;
        }
        const resolvedStartMinutes = resolvedStartTime ? timeToMinutes(resolvedStartTime) : timeSlot.time;
        const slotKey = `day-0-slot-${resolvedStartMinutes}`;

        console.log(
          `${emoji} ${label}: ${product.name || product.title} @ ${resolvedStartTime} (${timeSlot.duration}min)`
        );
        ...
        console.log(`   ${added ? '✅ Added' : '❌ Failed'}`);

        if (added) {
          usedProductIds.add(product.id);
          occupiedTimeSlots.push({ start: resolvedStartMinutes, duration: timeSlot.duration });
          activitiesAdded.push(`${label}: ${product.name || product.title}`);

          try {
            const availableProducts = validProducts.filter(p => !usedProductIds.has(p.id));
            console.log(`[Alternatives] Building for ${slotKey}, ${availableProducts.length} products available`);

            const alternatives = buildAlternativePool(
            ...
            console.log(
              `[Alternatives] Built ${alternatives.length} for ${label}:`,
              alternatives.map(a => `${a.name} (${a.score})`).join(', ')
            );

            if (alternatives.length > 0) {
              dispatch({
                type: ACTIONS.SET_ALTERNATIVES,
                payload: { slotKey, alternatives }
              });
            }
          } catch (err) {
      ...
      console.log('✅ Auto-generated activities:', activitiesAdded);

      if (activitiesAdded.length > 0) {
        showToast(`${activitiesAdded.length} activiteiten toegevoegd! Gebruik 🔄 om alternatieven te bekijken.`);
      } else if (validProducts.length > 0) {
        showToast('Geen activiteiten konden automatisch worden ingepland. Probeer handmatig toe te voegen.');
      } else {
        showToast('Geen geschikte activiteiten gevonden voor je voorkeuren. Pas je filters aan.');
      }
    })();
    ...
    state.plan.items.length,
    canonicalParticipants,
    addActivity, // Add dependency on addActivity function
    resolveStartTimeAgainstAvailability,
    showToast,
  ]);
    ...
    if (prefill.productId) {
      ...
      const pendingQueuedPrefillItems =
        Array.isArray(sessionPrefillRef.current) && sessionPrefillRef.current.length > 0
          ? sessionPrefillRef.current
          : [];
      const seededQueuedPrefillItems = Array.isArray(seededSessionPrefillRef.current)
        ? seededSessionPrefillRef.current
        : [];
      const matchesPrefillEntry = (entry) => {
        const queuedProductId = toPositiveInt(entry?.product_id ?? entry?.productId ?? entry?.id);
        if (queuedProductId !== prefill.productId) {
          return false;
        }

        const queuedDate = typeof entry?.date === "string" ? entry.date.trim() : "";
        const queuedTime = sanitiseTimeString(entry?.time);
        const sameDate = !prefill.date || !queuedDate || queuedDate === prefill.date;
        const sameTime = !prefill.time || !queuedTime || queuedTime === prefill.time;
        return sameDate && sameTime;
      };
      const hasMatchingPendingQueuedPrefill =
        isFreshProductPrefill && pendingQueuedPrefillItems.some(matchesPrefillEntry);
      const hasMatchingSeededQueuedPrefill =
        isFreshProductPrefill && seededQueuedPrefillItems.some(matchesPrefillEntry);

      if (hasMatchingPendingQueuedPrefill) {
        return;
      }

      if (hasMatchingSeededQueuedPrefill && state.plan.items.length > 0) {
        prefillPlanAppliedRef.current = true;
        return;
      }
      ...
      const product = state.products.find((entry) => entry.id === prefill.productId);
      if (!product) {
        return;
      }
      ...
      const startTime = resolvePrefillStartTime(prefill.time, product, state.config);
      const exists = state.plan.items.some(
        (item) =>
          item.productId === product.id && item.dayIndex === 0 && item.startTime === startTime
      );

      if (!exists) {
        const addActivityFn = addActivityRef.current;
        const prefillCombiItems = normalisePrefillCombiItems(prefill.combiItems);
        const added =
          typeof addActivityFn === "function" &&
          addActivityFn(
            { productId: product.id, dayIndex: 0, startTime },
            {
              locked: prefill.lockFirstSlot === true,
              resourceId:
                prefill.resourceId != null ? prefill.resourceId : product.resource_id ?? null,
              combiItems: prefillCombiItems,
              traceId: prefill.traceId ?? prefill.trace_id ?? null,
              source: "product-prefill",
            }
          );
  ...
  // Store theme generation flag
  const themeGeneratedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    console.log("[Planner] 🔍 useEffect triggered - checking prefill:", prefill);
    console.log("[Planner] 🔍 Config ready?", !!state.config, "| Config loading?", state.loading.config);
    console.log("[Planner] 🔍 Days:", state.plan.days.length, "| Items:", state.plan.items.length, "| Products:", state.products.length);
    
    const homeWidgetPrefill = (() => {
      if (typeof window === "undefined") {
        return null;
      }
      if (window.PreferenceManager && typeof window.PreferenceManager.load === "function") {
        const prefs = window.PreferenceManager.load();
        if (prefs) {
          return prefs;
        }
      }
      if (window.SBDP_HOME_WIDGET_PREFILL) {
        return window.SBDP_HOME_WIDGET_PREFILL;
      }
      try {
        const stored = sessionStorage.getItem("sbdp_home_widget_prefill");
        return stored ? JSON.parse(stored) : null;
      } catch (error) {
        return null;
      }
    ...
    if (!homeWidgetPrefill || !hasExplicitPlannerIngressPrefill(homeWidgetPrefill)) {
      console.log("[Planner] 🚫 Auto-fill preset generation is disabled");
      return;
    }

    if (homeWidgetAppliedRef.current) {
      console.log("[Planner] ✅ Home widget auto-fill already applied - skipping preset generation");
      return;
    }

    const effectivePrefill = {
      ...(prefill || {}),
      vibe: homeWidgetPrefill?.vibe ?? prefill?.vibe,
      audience: homeWidgetPrefill?.audience ?? prefill?.audience,
      duration: homeWidgetPrefill?.duration ?? prefill?.duration,
    };

    if (
      !effectivePrefill ||
      (!effectivePrefill.vibe && !effectivePrefill.audience && !effectivePrefill.duration)
    ) {
      console.log("[Planner] ❌ No prefill vibe/audience/duration detected");
      return;
    }
    if (!state.config || state.loading.config) {
      console.log("[Planner] ⏳ Config not ready yet");
      return;
    }
    if (!state.plan.days.length || state.plan.items.length > 0) {
      console.log("[Planner] ⏸️ Plan not ready or already has items");
      return;
    }
    if (!state.products.length) {
      console.log("[Planner] 📦 No products available yet");
      return;
    }
    if (themeGeneratedRef.current) {
      console.log("[Planner] ⏭️ Theme already generated, skipping");
      return;
    }

    const theme = resolveThemeFromHints(effectivePrefill);
    if (!theme) {
      console.log("[Planner] ❓ Could not resolve theme from hints:", effectivePrefill);
      return;
    }

    console.log(
      "[Planner] 🎨 Generating preset plan with theme:",
      theme,
      "from prefill:",
      effectivePrefill
    );
    themeGeneratedRef.current = true;
    
    // Call generatePresetPlan inline with intelligent scheduling
    const addFn = addActivityRef.current;
    if (typeof addFn !== "function") {
    ...
    const preferredTokens = themeTokensMap[theme] || [];
    
    // 🍽️ Always try to include a lunch option
    const lunchProducts = availableProducts.filter((product) => {
      const tokens = getProductCategoryTokens(product);
      const name = (product.name || "").toLowerCase();
      return tokens.some((token) => ["eten", "horeca", "lunch", "food"].includes(token)) ||
             name.includes("lunch") || name.includes("eten");
    });

    // Filter themed products (excluding lunch items for now)
    const themedProducts =
      preferredTokens.length > 0
        ? availableProducts.filter((product) => {
            const tokens = getProductCategoryTokens(product);
            const isLunch = lunchProducts.some(l => l.id === product.id);
            return !isLunch && tokens.some((token) => preferredTokens.includes(token));
          })
        : availableProducts.filter(p => !lunchProducts.some(l => l.id === p.id));

    // Build balanced selection: ochtend activity → lunch → middag/avond activities
    const selectedProducts = [];
    const seen = new Set();

    // 1. Morning activity (lighter/cultural)
    const morningProduct = themedProducts.find(p => !seen.has(p.id));
    if (morningProduct) {
      selectedProducts.push({ product: morningProduct, slot: 'morning' });
      seen.add(morningProduct.id);
    }

    // 2. Lunch slot (always around 12:00-13:00)
    if (lunchProducts.length > 0) {
      const lunchProduct = lunchProducts.find(p => !seen.has(p.id));
      if (lunchProduct) {
        selectedProducts.push({ product: lunchProduct, slot: 'lunch' });
        seen.add(lunchProduct.id);
      }
    }

    // 3. Afternoon/evening activities
    const remainingSlots = 4 - selectedProducts.length;
    for (let i = 0; i < remainingSlots; i++) {
      const nextProduct = themedProducts.find(p => !seen.has(p.id)) || 
                          availableProducts.find(p => !seen.has(p.id));
      if (nextProduct) {
        selectedProducts.push({ product: nextProduct, slot: i === 0 ? 'afternoon' : 'evening' });
    ...
    // Calculate smart start times based on slot type
    const timeSlots = {
      morning: 540,    // 09:00
      lunch: 720,      // 12:00  
      afternoon: 840,  // 14:00
    ...
    void (async () => {
      ...
      for (const { product, slot } of selectedProducts) {
        const baseTime = timeSlots[slot] || 540;
        const startTime = minutesToTime(
          baseTime + (slot === 'afternoon' || slot === 'evening' ? addedCount * 30 : 0)
        );
        const resolvedStartTime = await resolveStartTimeAgainstAvailability({
          product,
          desiredStartTime: startTime,
  ...
  const clearPlan = useCallback(() => {
    ...
    homeWidgetAppliedRef.current = true;
    homeWidgetStartRef.current = true;
    themeGeneratedRef.current = false;
  }, [dispatch]);
  ...
  const regeneratePlan = useCallback(
    (newPreferences) => {
      ...
      homeWidgetStartRef.current = false;
