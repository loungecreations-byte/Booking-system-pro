...
const FILTER_LABELS = {
duration: {
any: 'Alle duren', '60': 'Tot 60 min', '90': 'Tot 90 min', longer: 'Langer dan 90 min' },
price: {
all: 'Alle prijzen', low: '€0 - €25', mid: '€25 - €75', high: '€75+' } };
const CATEGORY_DEFAULT_LABEL = 'Alle categorieën';
const Planner = {
state: {
root: null,            restBase: '',            nonce: '',            date: null,            participants: 1,            products: [],            filtered: [],            items: [],            planId: null,            sessionId: null,            totals: {
...DEFAULT_TOTALS },            availability: {},            filter: {
query: '', duration: 'any', category: 'all', price: 'all' },            filterLabels: {
duration: FILTER_LABELS.duration.any, category: CATEGORY_DEFAULT_LABEL, price: FILTER_LABELS.price.all },            customer: {
name: '',                email: '',                phone: '',            },        },        ui: {},        priceTimer: null,        init(config) {
...
this.state.sessionId = config.sessionId || config.session_id || config.root.dataset.sessionId || null;
this.state.date = config.defaultDate || this.getToday();
this.buildLayout();
this.bindEvents();
console.log('[BPMPlanner] Initialising (session=%s)', this.state.sessionId);
...
this.state.customer = {
name: typeof name === 'string' ? name.trim() : '',                email: typeof email === 'string' ? email.trim() : '',                phone: typeof phone === 'string' ? phone.trim() : '',            };
...
this.ui.filterTriggers = Array.from(this.ui.filterBar.querySelectorAll('.bpm-filter__trigger'));
this.ui.filterMenus = {};
...
const menu = this.ui.filterBar.querySelector(`.bpm-filter__menu[data-filter-menu="${filterType}"]`);
if (menu) {
...
this.applyFilterSelection(filterType, value, target.textContent || '');
});
}
...
const options = [`<button type="button" data-value="all">${CATEGORY_DEFAULT_LABEL}</button>`];
categories.forEach((entry) => {
options.push(`<button type="button" data-value="${entry.slug}">${entry.label}</button>`);
});
...
const slugs = Array.isArray(product.category_slugs) ? product.category_slugs : [];
slugs.forEach((slug, index) => {
const normalisedSlug = typeof slug === 'string' ? slug.trim() : '';
if (!normalisedSlug || map.has(normalisedSlug)) {
return;
}
const label = typeof names[index] === 'string' && names[index].trim() !== '' ? names[index].trim() : normalisedSlug.replace(/[-_]/g, ' ');
map.set(normalisedSlug, { slug: normalisedSlug, label });
});
});
return Array.from(map.values()).sort((a, b) => a.label.localeCompare(b.label));
},        toggleFilterMenu(filterKey) {
if (!filterKey || !this.ui.filterMenus || !this.ui.filterMenus[filterKey]) {
return;
}
...
this.closeAllFilterMenus();
const trigger = this.ui.filterTriggers.find((button) => button.getAttribute('data-filter') === filterKey);
...
this.state.filterLabels.duration = FILTER_LABELS.duration[value] || FILTER_LABELS.duration.any;
}
else if (filterType === 'category') {
this.state.filter.category = value;
this.state.filterLabels.category = value === 'all' ? CATEGORY_DEFAULT_LABEL : (label && label.trim()) || CATEGORY_DEFAULT_LABEL;
}
else if (filterType === 'price') {
this.state.filter.price = value;
this.state.filterLabels.price = FILTER_LABELS.price[value] || FILTER_LABELS.price.all;
}
this.updateFilterTriggerLabels();
this.renderFilterTags();
this.closeAllFilterMenus();
...
trigger.setAttribute('aria-expanded', this.ui.openFilter === filterType ? 'true' : 'false');
});
},        renderFilterTags() {
...
tags.push({ type: 'duration', label: this.state.filterLabels.duration });
}
if (this.state.filter.category && this.state.filter.category !== 'all') {
tags.push({ type: 'category', label: this.state.filterLabels.category });
}
...
this.ui.filterTags.innerHTML = tags.map((tag) => `<button type="button" class="bpm-filter-tag" data-filter-remove="${tag.type}">${tag.label}<span aria-hidden="true">×</span></button>`).join('');
Array.from(this.ui.filterTags.querySelectorAll('button[data-filter-remove]')).forEach((button) => {
...
return {
name: (this.state.customer.name || '').trim(),                email: (this.state.customer.email || '').trim(),                phone: (this.state.customer.phone || '').trim(),            };
},        canSubmit() {
const info = this.getCustomerInfo();
return info.name !== '' && info.email !== '' && this.state.items.length > 0;
},        updateSubmitState() {
...
productsColumn.className = 'bpm-planner__column bpm-planner__column--products';
...
scheduleColumn.className = 'bpm-planner__column bpm-planner__column--schedule';
...
overviewColumn.className = 'bpm-planner__column bpm-planner__column--overview';
overviewColumn.innerHTML = `                <header class="bpm-planner__header">                    <h2>Overzicht</h2>                </header>                <div class="bpm-planner__field">                    <label for="bpm-participants">Deelnemers</label>                    <input id="bpm-participants" class="bpm-planner__participants" type="number" min="1" value="1">                </div>                <div class="bpm-planner__field">                    <label for="bpm-customer-name">Contactnaam</label>                    <input id="bpm-customer-name" class="bpm-planner__customer-name" type="text" placeholder="Naam contactpersoon">                </div>                <div class="bpm-planner__field">                    <label for="bpm-customer-email">E-mail</label>                    <input id="bpm-customer-email" class="bpm-planner__customer-email" type="email" placeholder="voorbeeld@mail.com">                </div>                <div class="bpm-planner__field">                    <label for="bpm-customer-phone">Telefoon</label>                    <input id="bpm-customer-phone" class="bpm-planner__customer-phone" type="tel" placeholder="06 12345678">                </div>                <div class="bpm-planner__totals">                    <div><span>Subtotaal</span><strong class="bpm-planner__total-subtotal">€0,00</strong></div>                    <div><span>Kosten</span><strong class="bpm-planner__total-fee">€0,00</strong></div>                    <div><span>Totaal</span><strong class="bpm-planner__total-total">€0,00</strong></div>                </div>                <div class="bpm-planner__actions">                    <button type="button" class="bpm-planner__btn bpm-planner__btn--calc">Herbereken</button>                    <button type="button" class="bpm-planner__btn bpm-planner__btn--save">Plan opslaan</button>                    <button type="button" class="bpm-planner__btn bpm-planner__btn--submit">Indienen</button>                </div>            `;
grid.appendChild(productsColumn);
grid.appendChild(scheduleColumn);
grid.appendChild(overviewColumn);
root.appendChild(grid);
this.ui = {
grid,                productsColumn,                scheduleColumn,                overviewColumn,                productList: productsColumn.querySelector('.bpm-planner__product-list'),                search: productsColumn.querySelector('.bpm-planner__search'),                filterBar: productsColumn.querySelector('.bpm-filter-bar'),                filterTags: productsColumn.querySelector('.bpm-filter-tags'),                itemsList: scheduleColumn.querySelector('.bpm-planner__items'),                dateInput: scheduleColumn.querySelector('.bpm-planner__date'),                participantsInput: overviewColumn.querySelector('.bpm-planner__participants'),                customerNameInput: overviewColumn.querySelector('.bpm-planner__customer-name'),                customerEmailInput: overviewColumn.querySelector('.bpm-planner__customer-email'),                customerPhoneInput: overviewColumn.querySelector('.bpm-planner__customer-phone'),                totalSubtotal: overviewColumn.querySelector('.bpm-planner__total-subtotal'),                totalFee: overviewColumn.querySelector('.bpm-planner__total-fee'),                totalTotal: overviewColumn.querySelector('.bpm-planner__total-total'),                calcButton: overviewColumn.querySelector('.bpm-planner__btn--calc'),                saveButton: overviewColumn.querySelector('.bpm-planner__btn--save'),                submitButton: overviewColumn.querySelector('.bpm-planner__btn--submit'),            };
...
this.state.participants = Number.isNaN(value) || value < 1 ? 1 : value;
event.target.value = String(this.state.participants);
...
const inputs = Array.isArray(this.ui.productDateInputs) ? this.ui.productDateInputs : [];
...
const participants = this.state.participants && this.state.participants > 0                ? this.state.participants                : 1;
params.append('participants', String(participants));
const query = params.toString();
const endpoint = `/products${query ? `?${query}` : ''}`;
return this.request(endpoint)                .then((products) => {
this.state.products = Array.isArray(products) ? products : [];
this.renderCategoryFilter();
this.updateFilterTriggerLabels();
this.filter(this.state.filter);
console.log(                        '[BPMPlanner] Products loaded (%d) voor %s',                        this.state.products.length,                        this.state.date || 'alle dagen'                    );
})                .catch((error) => {
...
this.request(`/plan?session_id=${encodeURIComponent(sessionId)}`)                .then((plan) => {
if (!plan) {
return;
}
...
if (plan.totals) {
this.state.totals = {
subtotal: plan.totals.subtotal || 0,                            fee: plan.totals.fee || 0,                            total: plan.totals.total || 0,                        };
}
if (plan.customer) {
this.setCustomerInfo(plan.customer.name || '', plan.customer.email || '', plan.customer.phone || '');
}
console.log('[BPMPlanner] Plan geladen (plan_id=%s)', this.state.planId);
this.render();
})                .catch((error) => {
...
if (!matchesCategory) {
const slugs = Array.isArray(product.category_slugs) ? product.category_slugs : [];
matchesCategory = slugs.some((slug) => typeof slug === 'string' && slug.toLowerCase() === categoryFilter.toLowerCase());
}
let matchesPrice = true;
const price = typeof product.price_pp === 'number' ? product.price_pp : parseFloat(product.price_pp || '0');
if (priceFilter === 'low') {
...
return matchesQuery && matchesDuration && matchesCategory && matchesPrice;
});
this.renderFilterTags();
this.updateFilterTriggerLabels();
this.renderProducts();
...
return this.request(`/availability?product_id=${productId}&date=${encodeURIComponent(date)}`)                .then((result) => {
const slots = result && Array.isArray(result.time_slots) ? result.time_slots : [];
...
product_id: productId,                start,                end: computedEnd,                duration,                title: product.title || `Product ${productId}`,                price_pp: Number.isFinite(price) ? price : 0,            });
this.state.items.sort((a, b) => a.start.localeCompare(b.start));
console.log('[BPMPlanner] Item toegevoegd (product=%s, start=%s)', productId, start);
this.renderItems();
this.renderTotals();
this.queuePriceUpdate();
},        removeItem(index) {
...
silent: isSilent, updateTotals: true }).then((validation) => {
...
const {
silent = false, updateTotals = false }
= options;
try {
const result = await this.request('/validate-plan', {
method: 'POST',                    body: this.buildPayload(),                });
if (!result) {
return {
ok: false, errors: [{
...
if (errors.length > 0) {
if (!silent) {
const message = errors                            .map((error) => (error && error.message) ? error.message : 'Plan bevat fouten')                            .join(', ');
...
this.state.totals = {
subtotal: result.totals.subtotal || 0,                        fee: result.totals.fee || 0,                        total: result.totals.total || 0,                    };
}
...
this.request('/plan', {
method: 'POST',                body: this.buildPayload(),            }).then((result) => {
if (!result) {
return;
}
...
this.applySessionId(this.state.sessionId);
this.render();
console.log('[BPMPlanner] Plan opgeslagen (plan_id=%s)', this.state.planId);
...
silent: false, updateTotals: true });
if (!validation.ok) {
const errors = validation.errors || [];
const message = errors.length                    ? errors.map((error) => (error && error.message) ? error.message : 'Onbekende fout').join(', ')                    : 'Plan bevat fouten.';
...
this.applySessionId(this.state.sessionId);
this.renderTotals();
console.log('[BPMPlanner] Plan verzonden (plan_id=%s)', this.state.planId);
...
const message = this.state.products.length === 0                    ? 'Geen activiteiten beschikbaar voor deze dag.'                    : 'Geen producten gevonden.';
...
const pricePerPerson = typeof product.price_pp === 'number' ? product.price_pp : parseFloat(product.price_pp || '0');
const participantCount = Number.isFinite(this.state.participants) && this.state.participants > 0 ? this.state.participants : null;
const totalPrice = participantCount ? pricePerPerson * participantCount : pricePerPerson;
const participantLabel = participantCount ? `${participantCount}×` : 'Kies deelnemers';
const categoryBadges = Array.isArray(product.categories) ? product.categories.filter((category) => typeof category === 'string' && category.trim() !== '').map((category) => `<span class="bpm-product__badge">${category}</span>`).join('') : '';
card.innerHTML = `                    <header class="bpm-product__header">                        <div class="bpm-product__title">                            <h3>${product.title || 'Onbekend product'}</h3>                            ${categoryBadges ? `<div class="bpm-product__badges">${categoryBadges}</div>` : ''}                        </div>                        <span class="bpm-product__meta">${durationLabel}</span>                    </header>                    <div class="bpm-product__pricing">                        <span class="bpm-product__price-pp">${this.formatCurrency(pricePerPerson)} <small>p.p.</small></span>                        <span class="bpm-product__price-total">${this.formatCurrency(totalPrice)} <small>${participantLabel}</small></span>                    </div>                    <div class="bpm-product__availability">                        <label class="bpm-product__date">                            <span class="screen-reader-text">Datum</span>                            <input type="date" class="bpm-product__date-picker" value="${this.state.date || ''}">                        </label>                        <div class="bpm-product__slot">                            <button type="button" class="bpm-product__slot-trigger">Beschikbare tijden</button>                            <div class="bpm-product__slot-menu" hidden></div>                        </div>                    </div>`;
...
event.preventDefault();
const dateValue = dateInput ? dateInput.value || this.state.date : this.state.date;
if (!dateValue) {
this.showError('Selecteer eerst een datum.');
return;
}
...
this.loadAvailability(product.id, dateValue)                        .then((slots) => {
...
slotMenu.innerHTML = '<p class="bpm-product__slots-empty">Geen vrije tijdsloten.</p>';
return;
}
const participants = this.state.participants > 0 ? this.state.participants : 1;
slots.forEach((slot) => {
const button = document.createElement('button');
button.type = 'button';
button.className = 'bpm-product__slot-btn';
const labelParts = [`${slot.start} - ${slot.end}`];
const capacityLeft = typeof slot.capacity_left === 'number' ? slot.capacity_left : null;
...
if (slot.price_pp !== undefined) {
const slotPrice = typeof slot.price_pp === 'number' ? slot.price_pp : parseFloat(slot.price_pp || '0');
if (!Number.isNaN(slotPrice) && slotPrice > 0) {
labelParts.push(`${this.formatCurrency(slotPrice * participants)}`);
}
}
...
list.innerHTML = '<li class="bpm-planner__empty">Nog geen activiteiten ingepland.</li>';
this.updateSubmitState();
return;
}
...
const pricePerPerson = typeof item.price_pp === 'number' ? item.price_pp : this.getProductPrice(item.product_id);
const participants = this.state.participants > 0 ? this.state.participants : 1;
...
min</small>                    </div>                    <div class="bpm-item__price">                        <strong>${this.formatCurrency(totalPrice)}</strong>                        <span>${this.formatCurrency(pricePerPerson)} p.p.</span>                    </div>                    <div class="bpm-item__actions">                        <button type="button" class="bpm-item__remove" aria-label="Verwijder item">&times;</button>                    </div>                `;
...
li.querySelector('.bpm-item__remove').addEventListener('click', () => this.removeItem(index));
list.appendChild(li);
});
this.updateSubmitState();
},        renderTotals() {
const participants = this.state.participants > 0 ? this.state.participants : 1;
const fallbackSubtotal = this.state.items.reduce((sum, item) => {
const price = typeof item.price_pp === 'number' ? item.price_pp : this.getProductPrice(item.product_id);
...
const subtotal = storedTotals.subtotal && storedTotals.subtotal > 0 ? storedTotals.subtotal : fallbackSubtotal;
const fee = storedTotals.fee && storedTotals.fee > 0 ? storedTotals.fee : 0;
const total = storedTotals.total && storedTotals.total > 0 ? storedTotals.total : (fallbackSubtotal + fee);
...
return {
plan_id: this.state.planId,                session_id: this.state.sessionId,                date: this.state.date,                participants: this.state.participants,                items: this.state.items.map((item) => ({
product_id: item.product_id,                    start: item.start,                    end: item.end,                })),                customer: this.getCustomerInfo(),            };
...
config.root = root;
config.restBase = config.rest_base || root.dataset.restBase || '/wp-json/booking/v1';
