<?php
/**
 * Elementor widget for displaying private tour meta.
 *
...
/**
 * Outputs core metadata for a private tour inside Elementor layouts.
 */
class SBDP_Elementor_Tour_Meta_Widget extends Widget_Base {
	...
	protected function render() {
		...
		if ( 'yes' === $settings['show_summary'] && '' !== trim( $summary ) ) {
			$output[] = sprintf(
				'<div class="sbdp-tour-meta__summary">%s</div>',
		...
		if ( 'yes' === $settings['show_duration'] && $duration > 0 ) {
			$meta_items[] = sprintf(
				'<span class="sbdp-tour-meta__chip">%s</span>',
				esc_html(
					sprintf(
		...
		if ( ! empty( $meta_items ) ) {
			$output[] = sprintf(
				'<div class="sbdp-tour-meta__chips">%s</div>',
				implode( '', $meta_items )
			);
		}
		...
		if ( 'yes' === $settings['show_support'] && is_email( $support_email ) ) {
			$output[] = sprintf(
				'<div class="sbdp-tour-meta__support">%s <a href="%s">%s</a></div>',
				esc_html__( 'Vragen?', 'sbdp' ),
				esc_url( 'mailto:' . $support_email ),
		...
		printf(
			'<div class="sbdp-elementor-tour-meta">%s</div>',
			implode( '', $output )
		);
	}
}
