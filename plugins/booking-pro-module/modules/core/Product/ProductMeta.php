...
final class ProductMeta {
	...
	public static function get_resource_ids( int $product_id ): array {
		...
		if ( is_string( $decoded ) ) {
			...
			if ( is_array( $decoded_json ) ) {
				return array_values( array_filter( array_map( 'intval', $decoded_json ) ) );
			}
		}
	...
	public static function get_resources_payload( int $product_id ): array {
		...
		foreach ( $ids as $resource_id ) {
			...
			if ( ! in_array( $post->post_type, array( 'bookable_resource', 'bsp_city_guide' ), true ) ) {
				continue;
			}

			$capacity = (int) get_post_meta( $resource_id, '_sbdp_resource_capacity', true );

			if ( $capacity <= 0 && 'bsp_city_guide' === $post->post_type ) {
				$capacity = (int) get_post_meta( $resource_id, '_bsp_cityguide_capacity', true );
			}
			...
			$out[] = array(
				...
				'capacity' => $capacity,
				'calendar_blocks'     => ResourceCalendar::get_calendar_blocks( $resource_id ),
				'calendar_last_sync'  => ResourceCalendar::get_last_sync( $resource_id ),
				'calendar_status'     => ResourceCalendar::get_status( $resource_id ),
			);
		}
