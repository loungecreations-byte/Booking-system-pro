...
final class SetupWizard
{
    ...
    /**
     * Retrieve preset definitions for external consumers.
     *
     * @return array<string,array<string,mixed>>
     */
    ...
    /**
     * Apply a preset to a specific product outside the admin UI.
     *
     ...
     * @return true|WP_Error
     */
    public static function apply_preset_to_product(int $product_id, string $preset_key, array $options = array())
    {
        return self::apply_preset($product_id, $preset_key, $options);
    }
    ...
    private static function sync_legacy_meta(int $product_id, array $meta): void
    {
        ...
        if (isset($meta['base_price_per_person'])) {
            $amount = is_numeric($meta['base_price_per_person']) ? round((float) $meta['base_price_per_person'], 2) : 0.0;
            if ($amount > 0) {
                update_post_meta($product_id, '_sbdp_price_per_person', $amount);
                update_post_meta($product_id, '_sbdp_supports_persons', 'yes');
            } else {
                delete_post_meta($product_id, '_sbdp_price_per_person');
                delete_post_meta($product_id, '_sbdp_supports_persons');
            }
        }
