(() => {
  ...
  const upload = async (root, state, mount, step, file, queuedId = "") => {
    ...
    try {
      ...
      if (result.status === "passed") {
        ...
        root.dispatchEvent(new CustomEvent("ddb:photo-challenge-passed", {
          detail: {
            ...result,
            moduleId: state.moduleId || "",
            moduleCompletion: result.rewards?.module_completion || {},
          },
        }));
  ...
  const sync = async (root) => {
    ...
    const photoModule = Array.isArray(step?.modules)
      ? step.modules.find((module) => module?.enabled && module?.type === "ai_photo_challenge")
      : null;
    if (!step || (step.type !== "photo_challenge" && !photoModule)) {
      stopCamera(state);
  ...
  const mountRoot = (root) => {
    ...
    const state = { steps: parseSteps(root), stream: null, stepId: null, moduleId: "", syncing: false, pendingFile: null, previewUrl: "", challenge: {} };
