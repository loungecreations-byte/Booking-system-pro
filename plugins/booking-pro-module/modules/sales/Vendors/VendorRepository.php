...
final class VendorRepository
{
    ...
    public function getVendorIdForProduct(int $productId): ?int
    {
        if (! $this->db instanceof wpdb) {
            return null;
        }

        $query = 'SELECT vendor_id FROM ' . $this->getProductTable() . ' WHERE product_id = %d LIMIT 1';
        $id    = (int) $this->db->get_var($this->db->prepare($query, absint($productId)));
    ...
    public function updateVendorMetadata(int $vendorId, array $metadata): bool
    {
        if (! $this->db instanceof wpdb) {
            return false;
        }

        $table = $this->getVendorTable();
        if ($table === '') {
            return false;
        }
