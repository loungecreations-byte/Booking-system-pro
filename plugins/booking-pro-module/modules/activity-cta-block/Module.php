...
final class Module implements ModuleInterface
{
    ...
    public function enqueueAssets(): void
    {
        ...
        if ($this->shouldBypassManagedProductPage() && ! $hasShortcode) {
            return;
        }
    ...
    private function ensureAssetsEnqueued(): void
    {
        ...
        if (\function_exists('wp_register_style')) {
            // SUPERSEDED BY ddb-core-ui listing-card-system.css — PENDING ARCHIVE
        }
    ...
    public function outputCtaViaAction(?int $postId = null): void
    {
        ...
        if ($postId === 0 || ! $this->shouldHandlePost($postId) || $this->shouldBypassManagedProductPage()) {
            return;
        }
    ...
    {
        if (! \function_exists('is_product') || ! \is_product() || $this->shouldBypassManagedProductPage()) {
            return;
        }
    ...
    private function shouldBypassManagedProductPage(): bool
    {
        if (\class_exists(ProductPageContext::class)) {
            return ProductPageContext::shouldUseLegacyPlannerOverrides();
        }

        return \function_exists('sbdp_should_output_product_planner_overrides')
            && \sbdp_should_output_product_planner_overrides();
    }
