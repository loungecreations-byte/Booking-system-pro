...
final class BookingsController
{
    ...
    public static function register(): void
    {
        ...
        register_rest_route(
            'bsp/v1',
            '/booking-board/bookings/partner-fallback',
            [
                'methods'             => 'POST',
                'callback'            => [__CLASS__, 'replacePartnerFallback'],
                'permission_callback' => [__CLASS__, 'canManage'],
            ]
        );
        ...
        register_rest_route(
            'bsp/v1',
            '/booking-board/bookings/dietary',
            [
                'methods'             => 'POST',
                'callback'            => [__CLASS__, 'updateDietaryProfiles'],
                'permission_callback' => [__CLASS__, 'canManage'],
            ]
        );
    ...
    public static function replacePartnerFallback(WP_REST_Request $request)
    {
        $payload = $request->get_json_params();
        if (! is_array($payload)) {
            $payload = [];
        ...
        return self::respond(static fn (BoardService $service) => $service->replacePartnerFallback($payload));
    }

    public static function updateDietaryProfiles(WP_REST_Request $request)
    {
        $payload = $request->get_json_params();
        if (! is_array($payload)) {
            $payload = [];
        ...
        return self::respond(static fn (BoardService $service) => $service->updateDietaryProfiles($payload));
    }
