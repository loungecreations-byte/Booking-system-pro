(function () {
    ...
    const canManageVendors = !!(config.capabilities && config.capabilities.manageVendors);
    const canManagePricing = !!(config.capabilities && (config.capabilities.managePricing || config.capabilities.manageVendors));
    const canManageChannels = !!(config.capabilities && (config.capabilities.manageChannels || config.capabilities.manageVendors));
    const canMutate = canManageVendors;
    ...
    const ChannelsApp = () => {
        ...
        const fetchChannels = async () => {
            ...
            try {
                ...
                setChannels(Array.isArray(payload && payload.channels) ? payload.channels : []);
            } catch (fetchError) {
                setError(fetchError && fetchError.message ? fetchError.message : __('Unable to load channels.', 'sbdp'));
            } finally {
                setLoading(false);
            }
        };
        ...
        const syncChannels = async (channelId = null) => {
            if (!canManageChannels) {
                return;
            }
            setSyncing(channelId === null ? 'all' : channelId);
            setError('');
            setMessage('');
            try {
                const body = channelId === null ? {} : { channel_id: channelId };
                await apiRequest('channels/sync', { method: 'POST', body });
                setMessage(channelId === null ? __('Channels synchronised.', 'sbdp') : __('Channel synchronised.', 'sbdp'));
                await fetchChannels();
            } catch (syncError) {
                setError(syncError && syncError.message ? syncError.message : __('Failed to synchronise.', 'sbdp'));
            } finally {
                setSyncing(null);
            }
        };
        ...
        if (loading) {
            return h('p', {}, __('Loading channels…', 'sbdp'));
        }

        return h(Fragment, {}, [
            error && h('div', { className: 'notice notice-error' }, h('p', {}, error)),
            message && h('div', { className: 'notice notice-success' }, h('p', {}, message)),
            canManageChannels && h('p', {}, h('button', {
                type: 'button',
                className: 'button button-primary',
                onClick: () => syncChannels(null),
                disabled: syncing !== null,
            }, syncing === 'all' ? __('Synchronising…', 'sbdp') : __('Synchronise all channels', 'sbdp'))),
            channels.length === 0
                ? h('p', {}, __('No channels configured yet.', 'sbdp'))
                : h('table', { className: 'widefat striped' }, [
                    h('thead', {}, h('tr', {}, [
                        h('th', {}, __('Name', 'sbdp')),
                        h('th', {}, __('Slug', 'sbdp')),
                        h('th', {}, __('Status', 'sbdp')),
                        h('th', {}, __('Commission', 'sbdp')),
                        canManageChannels && h('th', {}, __('Actions', 'sbdp')),
                    ].filter(Boolean))),
                    h('tbody', {}, channels.map((channel) => h('tr', { key: channel.id || channel.slug || channel.name }, [
                        h('td', {}, channel.name || channel.slug || __('Unknown', 'sbdp')),
                        h('td', {}, channel.slug || '–'),
                        h('td', {}, channel.active ? __('Active', 'sbdp') : __('Inactive', 'sbdp')),
                        h('td', {}, typeof channel.commission_rate === 'number'
                            ? `${channel.commission_rate.toFixed(2)}%`
                            : '—'),
                        canManageChannels && h('td', {}, h('button', {
                            type: 'button',
                            className: 'button button-secondary',
                            disabled: syncing !== null,
                            onClick: () => syncChannels(channel.id || channel.slug || null),
                        }, syncing === (channel.id || channel.slug || null) ? __('Synchronising…', 'sbdp') : __('Sync', 'sbdp'))),
                    ].filter(Boolean)))),
                ]),
        ]);
    };
    ...
    const PricingApp = () => {
        ...
        const [quoteForm, setQuoteForm] = useState({ productId: '', quantity: '1', channel: '' });
        ...
        const loadData = async () => {
            ...
            try {
                const [rulesPayload, presetsPayload] = await Promise.all([
                    apiRequest('yield/rules'),
                    apiRequest('yield/presets'),
                ]);
                const extractedRules = Array.isArray(rulesPayload && rulesPayload.rules) ? rulesPayload.rules : [];
                const extractedPresets = Array.isArray(presetsPayload && presetsPayload.presets) ? presetsPayload.presets : [];
                setRules(extractedRules);
                setPresets(extractedPresets);
                if (!selectedPreset && extractedPresets.length && extractedPresets[0].key) {
                    setSelectedPreset(String(extractedPresets[0].key));
                }
            } catch (fetchError) {
                setError(fetchError && fetchError.message ? fetchError.message : __('Unable to load pricing data.', 'sbdp'));
            } finally {
                setLoading(false);
            }
        };
        ...
        const applyPreset = async () => {
            if (!canManagePricing) {
                setError(__('You do not have permission to modify pricing presets.', 'sbdp'));
                return;
            }
            if (!selectedPreset) {
                setError(__('Select a preset to apply.', 'sbdp'));
                return;
            }
            setError('');
            setMessage('');
            try {
                await apiRequest('yield/presets', {
                    method: 'POST',
                    body: { key: selectedPreset },
                });
                setMessage(__('Preset applied. New rule created.', 'sbdp'));
                await loadData();
            } catch (presetError) {
                setError(presetError && presetError.message ? presetError.message : __('Failed to apply preset.', 'sbdp'));
            }
        };

        const requestQuote = async (event) => {
            event.preventDefault();
            if (!quoteForm.productId) {
                setError(__('Enter a product ID for a quote.', 'sbdp'));
                return;
            }
            setError('');
            setMessage('');
            ...
            setQuoteResult(null);
            try {
                const payload = await apiRequest('pricing/quote', {
                    method: 'POST',
                    body: {
                        product_id: parseInt(quoteForm.productId, 10),
                        quantity: parseInt(quoteForm.quantity || '1', 10) || 1,
                        channel: quoteForm.channel || undefined,
                    },
                });
                setQuoteResult(payload);
            } catch (quoteError) {
                setError(quoteError && quoteError.message ? quoteError.message : __('Quote request failed.', 'sbdp'));
            } finally {
                setQuoting(false);
            }
        };
        ...
        if (loading) {
            return h('p', {}, __('Loading pricing data…', 'sbdp'));
        }

        return h(Fragment, {}, [
            error && h('div', { className: 'notice notice-error' }, h('p', {}, error)),
            message && h('div', { className: 'notice notice-success' }, h('p', {}, message)),
            h('section', { className: 'bsp-pricing-section' }, [
                h('h2', {}, __('Yield rules', 'sbdp')),
                rules.length
                    ? h('table', { className: 'widefat striped' }, [
                        h('thead', {}, h('tr', {}, [
                            h('th', {}, __('ID', 'sbdp')),
                            h('th', {}, __('Name', 'sbdp')),
                            h('th', {}, __('Priority', 'sbdp')),
                            h('th', {}, __('Active', 'sbdp')),
                            h('th', {}, __('Created', 'sbdp')),
                        ])),
                        h('tbody', {}, rules.map((rule) => h('tr', { key: rule.id || rule.name }, [
                            h('td', {}, rule.id || '—'),
                            h('td', {}, rule.name || '—'),
                            h('td', {}, typeof rule.priority === 'number' ? rule.priority : '—'),
                            h('td', {}, rule.active ? __('Yes', 'sbdp') : __('No', 'sbdp')),
                            h('td', {}, rule.created_at || '—'),
                        ]))),
                    ])
                    : h('p', {}, __('No yield rules configured yet.', 'sbdp')),
            ]),
            h('section', { className: 'bsp-pricing-section' }, [
                h('h2', {}, __('Apply preset', 'sbdp')),
                presets.length
                    ? h('div', { className: 'bsp-form-inline' }, [
                        h('select', {
                            value: selectedPreset,
                            onChange: (event) => setSelectedPreset(event && event.target ? event.target.value : ''),
                            disabled: !canManagePricing,
                        }, presets.map((preset) => h('option', { value: preset.key || '', key: preset.key || preset.name }, preset.name || preset.key || 'Preset'))),
                        h('button', {
                            type: 'button',
                            className: 'button button-primary',
                            onClick: applyPreset,
                            disabled: !canManagePricing,
                        }, __('Apply preset', 'sbdp')),
                    ])
                    : h('p', {}, __('No presets available yet.', 'sbdp')),
            ]),
            h('section', { className: 'bsp-pricing-section' }, [
                h('h2', {}, __('Quick price quote', 'sbdp')),
                h('form', { onSubmit: requestQuote, className: 'bsp-form-inline' }, [
                    h('label', {}, [
                        __('Product ID', 'sbdp'),
                        h('input', {
                            type: 'number',
                            ...
                            onChange: (event) => setQuoteForm((prev) => ({ ...prev, productId: event.target.value })),
                            required: true,
                        }),
                    ]),
                    h('label', {}, [
                    ...
                    h('label', {}, [
                        __('Channel (optional)', 'sbdp'),
                        h('input', {
                            type: 'text',
                            value: quoteForm.channel,
                            onChange: (event) => setQuoteForm((prev) => ({ ...prev, channel: event.target.value })),
                        }),
                    ]),
                    h('button', {
                        type: 'submit',
                        className: 'button button-secondary',
                        disabled: quoting,
                    }, quoting ? __('Calculating…', 'sbdp') : __('Calculate', 'sbdp')),
                ]),
                quoteResult && h('div', { className: 'notice notice-info' }, [
                    h('p', {}, __('Adjusted price', 'sbdp') + ': ' + (quoteResult.adjusted_price ?? quoteResult.price ?? '—')),
                    Array.isArray(quoteResult.applied_rules) && quoteResult.applied_rules.length
                        ? h('p', {}, __('Applied rules:', 'sbdp') + ' ' + quoteResult.applied_rules.map((rule) => rule.name || rule.id).join(', '))
                        : null,
                ]),
            ]),
        ]);
    ...
    const PlaceholderApp = () => h('p', {}, __('This section is coming soon.', 'sbdp'));
    ...
    let AppComponent = VendorsApp;
    if (screen === 'channels') {
        AppComponent = ChannelsApp;
    } else if (screen === 'pricing') {
        AppComponent = PricingApp;
    } else if (screen === 'vendors') {
        AppComponent = VendorsApp;
    } else {
        AppComponent = PlaceholderApp;
    }

    if (rootFactory) {
        rootFactory(rootEl).render(h(AppComponent));
    } else if (typeof render === 'function') {
        render(h(AppComponent), rootEl);
    }
})();
