...
$phpFiles = Get-ChildItem -Path $Root -Recurse -Filter '*.php' -File |
    Where-Object {
        $_.FullName -notmatch '\\vendor\\' `
            ...
            -and $_.FullName -notmatch '\\.build-test\\' `
            -and $_.FullName -notmatch '\\build-test\\' `
...
$importantCheck = Join-Path $Root 'scripts/check-no-important.ps1'
if (Test-Path $importantCheck) {
    Write-Host "Running important-marker guard..." -ForegroundColor Cyan
    & $importantCheck
    if ($LASTEXITCODE -ne 0) {
        throw "important-marker guard reported issues."
    }
}
