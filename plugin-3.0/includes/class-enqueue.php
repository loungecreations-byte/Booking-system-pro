...
class SBDP_Enqueue {
    ...
    const FRONT_HANDLE_STYLE  = 'sbdp-planner';
    const FRONT_HANDLE_SCRIPT = 'sbdp-planner';
    const FRONT_HANDLE_VENDOR = 'sbdp-fullcalendar';
    ...
    /**
     * Enqueue frontend assets when the day planner is present.
     */
    ...
    /**
     * Ensure assets are present for Elementor editor/preview.
     */
    public static function enqueue_for_elementor() {
        self::register_front_assets();
        self::enqueue_front_assets();
    }

    /**
     * Load scripts and styles for admin applications.
     *
     * @param string $hook Current admin page hook.
     */
    public static function enqueue_admin_assets( $hook ) {
        if ( in_array( $hook, [ 'bookings_page_sbdp_availability', 'bookings_page_sbdp_pricing' ], true ) ) {
        ...
        if ( 'bookings_page_sbdp_scheduler' === $hook ) {
    ...
    /**
     * Enqueue and localise planner assets.
     */
    ...
    /**
     * Determine if the current request needs the planner assets.
     *
     * @return bool
     */
    private static function should_enqueue_planner() {
        if ( is_admin() ) {
            return false;
        }
    ...
    /**
     * Build the front-end configuration array passed to planner.js.
     *
     * @return array<string,mixed>
     */
    private static function get_frontend_config() {
    ...
    /**
     * Localised strings for the planner interface.
     *
     * @return array<string,string>
     */
    private static function get_i18n_strings() {
        return [
            'price_pp'                  => __( 'Prijs p.p.', 'sbdp' ),
            'duration'                  => __( 'Duur', 'sbdp' ),
    ...
    /**
     * Detect whether the current request is an Elementor preview/editor session.
     *
     * @return bool
     */
    ...
    /**
     * Determine if an Elementor document contains the shortcode or widget.
     *
     ...
     * @return bool
     */
    private static function elementor_document_contains_shortcode( $post_id, $shortcode ) {
        if ( ! $post_id ) {
            return false;
        }
        ...
        if ( is_string( $raw_data ) && false !== strpos( $raw_data, $shortcode ) ) {
            return true;
        }
    ...
    /**
     * Recursively inspect Elementor data for the shortcode usage.
     *
     ...
     * @return bool
     */
    private static function search_elementor_nodes_for_shortcode( $nodes, $shortcode ) {
        foreach ( $nodes as $node ) {
            ...
            if ( isset( $node['widgetType'] ) ) {
                ...
                if ( 'shortcode' === $widget_type ) {
                    ...
                    if ( is_string( $content ) && false !== strpos( $content, $shortcode ) ) {
                        return true;
                    }
                }
